import React from "react";

export default function MakingToken() {
  return (
    <div className="h-[100vh] w-[100%] flex flex-col justify-evenly items-center">
      <div className="h-[70vh] w-[50%]  flex flex-col justify-evenly items-center">
        <p className="h-[30vh] w-[50%] text-center text-4xl font-semibold text-[#434343]">
          YOUR FORM <br /> HAS SUCCESFULLY BEEN MADE
        </p>
        <p className="font-semibold text-[#434343] text-4xl mt-[100px]">
          009876ZX
        </p>
        <button className="h-[8vh] w-[14%] bg-[#D9D9D9] rounded-lg mt-5">
          Copy
        </button>
        <a href="#" className="mt-5">
          tap here to return to home page
        </a>
      </div>
    </div>
  );
}
